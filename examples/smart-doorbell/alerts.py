...
* the ``AlertSource.name`` default of ``"smart-doorbell"`` — the SDK
...
class AlertDispatcher(_SdkAlertDispatcher):
    ...
    ``smart_doorbell.py`` (and any operator forks of it) don't need a
    rename to ride the SDK."""
...
# This process is the smart-doorbell app — see module docstring.
set_default_source(kind="app", name="smart-doorbell", version="1.0.0")
