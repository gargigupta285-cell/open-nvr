...
"""
Point-in-polygon test for loitering-zone evaluation.
...
the loitering-dwell timer for that camera.

Copied verbatim from ``examples/intrusion-detection/zone.py`` — the
two examples deliberately share this helper as the canonical
"polygon math is per-camera business logic, not framework-y enough
to factor into a library" template. Community contributors writing
their own apps copy this file alongside (see README for the design
intent).
...
def _point_in_polygon(point: Point, polygon: Sequence[Point]) -> bool:
    ...
    treated as INSIDE — the safer choice for a loitering detector
    (we'd rather time-track a borderline person than miss them)."""
    n = len(polygon)
    inside = False
    j = n - 1
    for i in range(n):
