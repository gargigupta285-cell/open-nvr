...
Copied from ``examples/intrusion-detection/alerts.py`` per the
"copy-as-template" model documented in both examples' READMEs;
defaults tweaked so the §11.5 ``source.name`` reports the right
app to downstream consumers (operators filtering by source need
the correct value here, not the intrusion-detection literal).
