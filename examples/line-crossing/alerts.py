...
``AlertSource.name`` default of ``"line-crossing"``. The SDK replaces
the hardcoded default with a process-wide setting; declaring it here
keeps this process's alerts stamped with the right §11.5 source
identity (the ``Detector`` base re-asserts it from the manifest at
construction time).
"""
...
# This process is the line-crossing app — see module docstring.
set_default_source(kind="app", name="line-crossing", version="1.0.0")
