...
copy-as-template model). Per §08 of the App SDK spec, that canonical
implementation now lives in the ``opennvr-app-sdk`` package; this
module re-exports it so existing imports (``from alerts import
Alert, ...``) keep working unchanged.

Two local touches survive the move:

* the ``AlertSource.name`` default of ``"license-plate-recognition"``
  — the SDK replaces the old hardcoded per-copy default with a
  process-wide setting, asserted here (and re-asserted from the
  manifest by the ``FrameApp`` base at construction time);
* ``AlertDispatcher.dispatch`` — this example historically called the
  ...
  and its tests stay source-compatible. The local ``build_dispatcher``
  also keeps this example's ``webhook_timeout_seconds`` knob, which
  the SDK factory doesn't expose.
"""
...
class AlertDispatcher(_SdkAlertDispatcher):
    ...
    ``license_plate_recognition.py`` (and any operator forks of it)
    don't need a rename to ride the SDK."""
...
def build_dispatcher(
    *,
    ...
    the ``dispatch`` alias is available and honoring this example's
    webhook timeout knob."""
    channels: list[AlertChannel] = [StdoutChannel()]
    if webhook_url:
        channels.append(
            WebhookChannel(webhook_url, timeout_seconds=webhook_timeout_seconds)
        )
    if nats_alerts_url:
        channels.append(
            NatsAlertChannel(
...
# This process is the license-plate-recognition app — see module docstring.
set_default_source(kind="app", name="license-plate-recognition", version="1.0.0")
