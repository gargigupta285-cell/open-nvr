...
and their scheme factory. Per §08 step 5 of the App SDK spec, the
canonical implementation now lives in the ``opennvr-app-sdk`` package;
this module re-exports it so existing imports (``from frame_sources
import FrameSource, build_frame_source, ...``) keep working unchanged.
...
# the old copy — downstream forks' tests monkeypatch it.
